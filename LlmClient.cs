...
public static class LlmClient
{
    ...
    /// </summary>
    /// <summary>按给定要求直接问一句。游戏里识别指令用这个。</summary>
    public static Task<string> AskAsync(string user) => CompleteAsync(user, chat: false);

    public static Task<string> ChatAsync(string scene, string situation = null)
    {
        string user = "现在发生的互动:" + scene;
        if (!string.IsNullOrWhiteSpace(situation)) user += "\n当前状态:" + situation;
        user += "\n请只用一句中文台词回应。";
        return CompleteAsync(user, chat: true);
    }

    static async Task<string> CompleteAsync(string user, bool chat)
    {
        ...
        try
        {
            ...
            string sys = chat
                ? (string.IsNullOrWhiteSpace(cfg.llmPersona)
                    ? "你是一只桌面宠物,性格活泼可爱,称呼使用者为主人。"
                    : cfg.llmPersona).Replace("{name}", cfg.petName)
                  + "\n只用简体中文,只说一句,不超过40字,不要引号和旁白。你的名字叫「" + cfg.petName + "」。"
                : "你是我的世界服务器管理员。只按用户的要求输出,不要多说。";
            ...
            var payload = new
            {
                ...
                temperature = chat ? cfg.llmTemperature : 0,
                max_tokens = 120,
                stream = false
            };
